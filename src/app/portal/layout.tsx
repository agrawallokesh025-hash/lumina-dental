  ...
  return (
    <div className="min-h-screen bg-background text-white flex flex-col md:flex-row pt-20 md:pt-24 pb-12 relative overflow-x-hidden">
      ...
      <aside className="w-full md:w-64 border-b md:border-b-0 md:border-r border-white/10 px-4 md:px-6 py-4 md:py-8 flex flex-row md:flex-col relative md:fixed h-auto md:h-[calc(100vh-6rem)] md:top-24 bg-background/90 md:bg-background/50 backdrop-blur-xl z-20 items-center md:items-stretch justify-between">
        <h2 className="font-serif text-2xl font-bold mb-10 text-white hidden md:block">Patient Portal</h2>
        ...
        <nav className="flex flex-row md:flex-col flex-1 overflow-x-auto no-scrollbar gap-2 md:gap-0 md:space-y-4 font-sans text-sm">
          <Link href="/portal" className={`flex items-center gap-2 md:gap-3 px-4 py-2 md:py-3 rounded-xl transition-all whitespace-nowrap ${pathname === '/portal' ? 'bg-white/10 text-white border border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
            <Calendar className="w-4 h-4 md:w-5 md:h-5" /> My Appointments
          </Link>
          <Link href="/" className="flex items-center gap-2 md:gap-3 px-4 py-2 md:py-3 rounded-xl transition-all text-slate-400 hover:text-white hover:bg-white/5 whitespace-nowrap">
            <Home className="w-4 h-4 md:w-5 md:h-5" /> Back to Home
          </Link>
        </nav>
